...
mawk -f bench.noselfhit.awk scop_lookup.fix.tsv <(for file in ${results_dir}/*; do sort -g -k 1,1 -k 4,4gr "$file"; done) > ${output_file}

# Calculate number of lines in the concatenated input files and num_lines / 11209
num_lines=$(cat ${results_dir}/* | wc -l)
division_result=$(echo "$num_lines / 11209" | bc -l)
...
scaling_factor=$(echo "3566 * (11209 / $num_lines)" | bc -l)
